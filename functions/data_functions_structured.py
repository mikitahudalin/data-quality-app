...
def init_session():
    ...
    if 'Q&AButtonState' not in st.session_state:
        st.session_state['Q&AButtonState'] = None
...
def mongo_load(collection, task_ids):
    ...
    if task_id_list:
        ...
        st.session_state['Listings'] = pd.DataFrame(db[collection].find({"task_id": {"$in": task_id_list},
                                                                        "data_type": 'listings'}))
        st.session_state['Content'] = pd.DataFrame(db[collection].find({"task_id": {"$in": task_id_list},
                                                                        "data_type": 'content'}))
        st.session_state['Reviews'] = pd.DataFrame(db[collection].find({"task_id": {"$in": task_id_list},
                                                                        "data_type": 'reviews'}))
        st.session_state['Q&A'] = pd.DataFrame(db[collection].find({"task_id": {"$in": task_id_list},
                                                                        "data_type": 'qa'}))
